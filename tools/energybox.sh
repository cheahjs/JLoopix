...
for folder in ../results/bandwidth/*; do
    ...
    java -jar energybox-2.0.jar --t="$folder/energybox.pcap" --n=3g_teliasonera.config --d=nexus_one_3g.config --f="$folder/energybox" | grep "Total power in Joules" | awk -F " " '{print $4}' > $folder/energy.txt
done